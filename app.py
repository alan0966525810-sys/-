...
def init_db():
    ...
    if not cur.execute("SELECT 1 FROM USER WHERE username='admin'").fetchone():
        cur.execute("INSERT INTO USER VALUES (NULL,'admin',?,'admin',NULL,?)",
                    (hp('admin123'), nows()))
...
def do_register(username, password, email):
    db = conn()
    try:
        db.execute("INSERT INTO USER VALUES (NULL,?,'%s','user',?,?)" % hp(password),
                   (username, email, nows()))
        db.commit(); db.close()
        return True, '註冊成功'
...
def do_register(username, password, email):
    ...
    try:
        db.execute("INSERT INTO USER VALUES (NULL,?,?,'user',?,?)",
                   (username, hp(password), email, nows()))
        db.commit(); db.close()
        return True, '註冊成功'
    except sqlite3.IntegrityError:
        db.close(); return False, '此帳號已存在'
...
def book_room(user_id, room_id, book_date, st_, et_, note):
    if room_conflict(room_id, book_date, st_, et_):
        return False, '此時段已有人預約，請選擇其他時段'
    db = conn()
    db.execute("INSERT INTO ROOM_BOOKING VALUES (NULL,?,?,?,?,?,'confirmed',?,?)",
               (user_id, room_id, book_date, st_, et_, note, nows()))
...
def modify_room(booking_id, user_id, room_id, book_date, st_, et_, note):
    if room_conflict(room_id, book_date, st_, et_, exclude=booking_id):
        return False, '此時段已有人預約'
    db = conn()
    db.execute("UPDATE ROOM_BOOKING SET book_date=?,start_time=?,end_time=?,note=? WHERE booking_id=? AND user_id=?",
               (book_date, st_, et_, note, booking_id, user_id))
    db.commit(); db.close(); return True, '修改成功'

def all_room_bookings():
    db = conn()
    rows = db.execute("""SELECT rb.*, r.name AS room_name, u.username FROM ROOM_BOOKING rb
...
def book_equip(user_id, equip_id, book_date, st_, et_, note):
    db = conn()
    db.execute("INSERT INTO EQUIP_BOOKING VALUES (NULL,?,?,?,?,?,'borrowed',NULL,?,?)",
...
def all_equip_bookings():
    db = conn()
    rows = db.execute("""SELECT eb.*, e.name AS equip_name, u.username FROM EQUIP_BOOKING eb
...
def get_all_users():
    db = conn()
    rows = db.execute("SELECT user_id,username,role,email,created_at FROM USER ORDER BY created_at DESC").fetchall()
    db.close(); return [dict(r) for r in rows]
                ...
                if st.form_submit_button('✅ 確認預約', use_container_width=True):
                    if st_ >= et_:
                        ...
                        ok, msg = book_room(user['user_id'], rm[sel],
                                            str(bd), str(st_)[:5], str(et_)[:5], note)
                        if ok:
...
def page_history():
    ...
    with tab1:
        ...
        if not bks:
            ...
            df = pd.DataFrame(bks)[['room_name','book_date','start_time','end_time','status','note','created_at']]
            df.columns = ['教室','日期','開始','結束','狀態','備註','建立時間']
            df['狀態'] = df['狀態'].map(SL)
...
def page_admin():
    ...
    with tabs[2]:
        ...
        if not bks:
            ...
            df = pd.DataFrame(bks)[['booking_id','username','room_name','book_date','start_time','end_time','status','note']]
            df.columns = ['ID','使用者','教室','日期','開始','結束','狀態','備註']
            df['狀態'] = df['狀態'].map(SL)
            st.dataframe(df, use_container_width=True, hide_index=True)
            st.divider()
    ...
    with tabs[4]:
        users = get_all_users()
        df = pd.DataFrame(users)[['username','role','email','created_at']]
        df.columns = ['帳號','角色','Email','註冊時間']
        st.dataframe(df, use_container_width=True, hide_index=True)
