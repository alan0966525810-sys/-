...
def render_room_calendar(room, week_offset_key='week_offset', compact=False):
    """
    渲染指定教室的週課表 HTML。
    compact=True 時使用較窄欄寬，適合側欄顯示。
    week_offset_key：session_state 的週偏移 key，不同頁面用不同 key 避免互相干擾。
    """
    today = date.today()
    ...
    COL_W         = 72 if compact else 110
    ROW_H         = 18 if compact else 20
    TIME_W        = 44
    total_h       = total_slots * ROW_H
    header_h      = 36

    C_FIXED   = ('#1e3a5f', '#d6e8ff')
    C_BOOKING = ('#3b1f00', '#ffe8c0')
    C_PENDING = ('#4a0060', '#f0d6ff')
    C_TODAY   = '#fffbe6'
    C_WEEKEND = '#f8f8f8'
    C_NORMAL  = '#ffffff'
    C_BORDER  = '#dde1e7'

    def block(top, h, bg, fg, lbl, sub=''):
    ...
    time_axis = ''.join(
        ...
        f"font-size:9px;color:#999;text-align:right;padding-right:5px;line-height:1'>"
        f"{slot_label(s)}</div>"
    ...
    parts = [f"""<div style='display:flex;font-family:sans-serif;
      ...
      <div style='flex:0 0 {TIME_W}px;background:#f5f6f8;border-right:1px solid {C_BORDER}'>
        <div style='height:{header_h}px;border-bottom:1px solid {C_BORDER}'></div>
        <div style='position:relative;height:{total_h}px'>{time_axis}</div>
      </div>"""]

    for i, (d, ds) in enumerate(zip(week_dates, week_strs)):
        is_today   = (d == today)
        is_weekend = (i >= 5)
        bg_col     = C_TODAY if is_today else (C_WEEKEND if is_weekend else C_NORMAL)
        border     = '2px solid #f0a500' if is_today else f'1px solid {C_BORDER}'
        hdr_bg     = '#fff8e1' if is_today else bg_col
        day_color  = '#e67e00' if is_today else ('#aaa' if is_weekend else '#333')
        ...
        for fc in fc_by_day[i]:
            ...
            if e > s:
                blks.append(block(s*ROW_H, (e-s)*ROW_H,
                                  C_FIXED[1], C_FIXED[0],
                                  fc['title'], f"{fc['start_time']}~{fc['end_time']}"))
        for b in bk_by_date.get(ds, []):
            ...
            if e > s:
                if b['status'] == 'pending_return':
                    bg2, fg2, lbl = C_PENDING[1], C_PENDING[0], f"🔔{b['username']}"
                else:
                    bg2, fg2, lbl = C_BOOKING[1], C_BOOKING[0], f"📌{b['username']}"
                blks.append(block(s*ROW_H, (e-s)*ROW_H, bg2, fg2, lbl,
                                  b.get('purpose') or ''))

        grid_lines = ''.join(
            f"<div style='position:absolute;top:{s*ROW_H}px;left:0;right:0;"
            f"height:1px;background:{C_BORDER};opacity:.5'></div>"
            for s in range(0, total_slots + 1, 2)
        )
        parts.append(f"""
      <div style='flex:1;min-width:{COL_W}px;border-left:{border};box-sizing:border-box'>
        <div style='height:{header_h}px;background:{hdr_bg};display:flex;flex-direction:column;
                    align-items:center;justify-content:center;
                    border-bottom:1px solid {C_BORDER}'>
          <div style='font-size:11px;font-weight:700;color:{day_color}'>週{WEEKDAY_ZH[i]}</div>
          <div style='font-size:10px;color:#999'>{d.month}/{d.day}</div>
        </div>
        <div style='position:relative;height:{total_h}px;background:{bg_col}'>
          {grid_lines}{''.join(blks)}
        </div>
      </div>""")
    ...
    st.markdown(
        ...
        f"<span style='background:{C_FIXED[1]};color:{C_FIXED[0]};padding:1px 7px;"
        f"border-radius:3px;font-weight:600'>固定課程</span>"
        f"<span style='background:{C_BOOKING[1]};color:{C_BOOKING[0]};padding:1px 7px;"
        f"border-radius:3px;font-weight:600'>📌 已預約</span>"
        f"<span style='background:{C_PENDING[1]};color:{C_PENDING[0]};padding:1px 7px;"
        f"border-radius:3px;font-weight:600'>🔔 申請歸還中</span>"
        "</div>",
        unsafe_allow_html=True
    )
                    ...
                    if st.form_submit_button('✅ 確認預約', use_container_width=True):
                        if st_ >= et_:
                            ...
                            ok, msg = book_room(user['user_id'], room['room_id'],
                                                ...
                                                final_purpose, note)
                            if ok:
                                st.success(f'✅ {msg}！{sel_room}　{bd}　{str(st_)[:5]} ~ {str(et_)[:5]}')
                                st.balloons()
                            else:
                                st.error(msg)
