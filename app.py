...
def get_fixed_courses(room_id):
    db = conn()
    rows = db.execute(
        "SELECT * FROM FIXED_COURSE WHERE room_id=? ORDER BY weekday, start_time",
        (room_id,)
    ).fetchall()
    db.close(); return [dict(r) for r in rows]
...
def add_fixed_course(room_id, weekday, start_time, end_time, title, note):
    db = conn()
    db.execute(
        "INSERT INTO FIXED_COURSE (room_id,weekday,start_time,end_time,title,note) VALUES (?,?,?,?,?,?)",
...
def get_week_bookings(room_id, dates):
    """回傳某教室在給定日期清單內的所有預約（confirmed / pending_return）"""
    db = conn()
    placeholders = ','.join('?' * len(dates))
    rows = db.execute(
        f"""SELECT rb.*, u.username, u.identity FROM ROOM_BOOKING rb
            JOIN USER u ON rb.user_id=u.user_id
            WHERE rb.room_id=? AND rb.book_date IN ({placeholders})
              AND rb.status IN ('confirmed','pending_return')
...
def page_query():
    ...
    with tab1:
        ...
        # 教室選擇 + 週切換
        col_sel, col_prev, col_cur, col_next = st.columns([3, 1, 2, 1])
        sel_room = col_sel.selectbox('選擇教室', list(rm_map.keys()), key='qr_sel', label_visibility='collapsed')
        ...
        if col_prev.button('◀ 上週', key='prev_week'):
            st.session_state.week_offset -= 1
            st.rerun()
        if col_next.button('下週 ▶', key='next_week'):
            st.session_state.week_offset += 1
            st.rerun()
        if col_cur.button('回本週', key='cur_week'):
            st.session_state.week_offset = 0
            st.rerun()
        ...
        # 整理成 dict：date_str → list of events
        bk_by_date = {d: [] for d in week_strs}
        for b in bookings:
            bk_by_date[b['book_date']].append(b)

        # 固定課程：weekday 0=週一
        fc_by_day = {i: [] for i in range(7)}
        for fc in fixed_crs:
            fc_by_day[fc['weekday']].append(fc)

        # 時間軸設定（07:00 ~ 22:00，每格30分鐘）
        HOUR_START = 7
        HOUR_END   = 22
        SLOT_MIN   = 30
        ...
        def slot_to_label(s):
            total_m = HOUR_START * 60 + s * SLOT_MIN
        ...
        # ── 產生 HTML 課表 ───────────────────────────────────
        COL_W   = 120   # 每天欄寬 px
        ROW_H   = 20    # 每 slot 高度 px
        TIME_W  = 52    # 時間軸欄寬 px
        total_h = total_slots * ROW_H

        # 色票
        COLOR_FIXED   = ('#1e3a5f', '#d6e8ff')   # 固定課程  (字, 背景)
        COLOR_BOOKING = ('#3b1f00', '#ffe8c0')    # 預約
        COLOR_PENDING = ('#4a0060', '#f0d6ff')    # 申請歸還中
        COLOR_TODAY   = '#fffbe6'
        COLOR_WEEKEND = '#f8f8f8'
        COLOR_NORMAL  = '#ffffff'
        COLOR_BORDER  = '#dde1e7'

        def make_block(top, height, bg, fg, label, sublabel=''):
            sub = f"<div style='font-size:10px;opacity:.75;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap'>{sublabel}</div>" if sublabel else ''
            return (
                f"<div style='position:absolute;top:{top}px;left:2px;right:2px;height:{height-3}px;"
                f"background:{bg};color:{fg};border-radius:4px;padding:3px 5px;"
                f"font-size:11px;font-weight:600;overflow:hidden;z-index:2;box-shadow:0 1px 3px rgba(0,0,0,.15)'>"
                f"<div style='overflow:hidden;text-overflow:ellipsis;white-space:nowrap'>{label}</div>{sub}</div>"
            )
        ...
        for i, (d, ds) in enumerate(zip(week_dates, week_strs)):
            is_today   = (d == today)
            is_weekend = (i >= 5)
            bg_col     = COLOR_TODAY if is_today else (COLOR_WEEKEND if is_weekend else COLOR_NORMAL)

            blocks = []

            # 固定課程
            for fc in fc_by_day[i]:
                s = max(0, time_to_slot(fc['start_time']))
                e = min(total_slots, time_to_slot(fc['end_time']))
                if e > s:
                    blocks.append(make_block(s*ROW_H, (e-s)*ROW_H,
                                             COLOR_FIXED[1], COLOR_FIXED[0],
                                             fc['title'],
                                             f"{fc['start_time']}~{fc['end_time']}"))

            # 預約
            for b in bk_by_date.get(ds, []):
                s = max(0, time_to_slot(b['start_time']))
                e = min(total_slots, time_to_slot(b['end_time']))
                if e > s:
                    if b['status'] == 'pending_return':
                        bg, fg = COLOR_PENDING
                        lbl = f"🔔 {b['username']}"
                    else:
                        bg, fg = COLOR_BOOKING
                        lbl = f"📌 {b['username']}"
                    sub = b.get('purpose') or ''
                    blocks.append(make_block(s*ROW_H, (e-s)*ROW_H, bg, fg, lbl, sub))

            col_htmls.append((bg_col, blocks, is_today, d))

        # 時間軸 HTML
        time_labels = ''.join(
            f"<div style='position:absolute;top:{s*ROW_H}px;left:0;width:{TIME_W}px;"
            f"font-size:10px;color:#888;text-align:right;padding-right:6px;line-height:1'>"
            f"{slot_to_label(s)}</div>"
            for s in range(0, total_slots+1, 2)   # 每小時一個標籤
        )

        # 整張課表 HTML
        header_h = 38
        total_canvas_h = total_h + header_h + 8

        html_parts = [f"""
        <div style='display:flex;font-family:sans-serif;border:1px solid {COLOR_BORDER};
                    border-radius:8px;overflow:hidden;width:100%;box-sizing:border-box'>
          <!-- 時間軸 -->
          <div style='flex:0 0 {TIME_W}px;background:#f5f6f8;border-right:1px solid {COLOR_BORDER}'>
            <div style='height:{header_h}px;border-bottom:1px solid {COLOR_BORDER}'></div>
            <div style='position:relative;height:{total_h}px'>{time_labels}</div>
          </div>
        """]

        for (bg_col, blocks, is_today, d) in col_htmls:
            day_i = d.weekday()
            border_style = '2px solid #f0a500' if is_today else f'1px solid {COLOR_BORDER}'
            header_bg = '#fff8e1' if is_today else bg_col
            day_lbl  = WEEKDAY_ZH[day_i]
            date_lbl = f"{d.month}/{d.day}"
            blocks_html = ''.join(blocks)
            html_parts.append(f"""
          <div style='flex:1;min-width:{COL_W}px;border-left:{border_style};box-sizing:border-box'>
            <div style='height:{header_h}px;background:{header_bg};display:flex;flex-direction:column;
                        align-items:center;justify-content:center;border-bottom:1px solid {COLOR_BORDER}'>
              <div style='font-size:12px;font-weight:700;color:{"#e67e00" if is_today else "#333"}'>{day_lbl}</div>
              <div style='font-size:11px;color:#888'>{date_lbl}</div>
            </div>
            <div style='position:relative;height:{total_h}px;background:{bg_col}'>
              {''.join(f"<div style='position:absolute;top:{s*ROW_H}px;left:0;right:0;height:1px;background:{COLOR_BORDER};opacity:.5'></div>" for s in range(0, total_slots+1, 2))}
              {blocks_html}
            </div>
          </div>
            """)
        ...
        st.markdown(
            f"<div style='overflow-x:auto'>{full_html}</div>",
        ...
        st.markdown(
            ...
            f"<span style='background:{COLOR_FIXED[1]};color:{COLOR_FIXED[0]};padding:2px 8px;border-radius:4px;font-weight:600'>固定課程</span>"
            f"<span style='background:{COLOR_BOOKING[1]};color:{COLOR_BOOKING[0]};padding:2px 8px;border-radius:4px;font-weight:600'>📌 已預約</span>"
            f"<span style='background:{COLOR_PENDING[1]};color:{COLOR_PENDING[0]};padding:2px 8px;border-radius:4px;font-weight:600'>🔔 申請歸還中</span>"
            f"<span style='background:{COLOR_TODAY};padding:2px 8px;border-radius:4px'>今日</span>"
            "</div>",
            unsafe_allow_html=True
        )
...
def page_admin():
    ...
    with tabs[0]:
        ...
        st.divider()
        st.subheader('📅 管理固定課程')
        st.caption('固定課程會顯示在週課表上，讓使用者知道哪些時段本來就有課。')

        rooms_for_fc = get_rooms()
        if rooms_for_fc:
            fc_rm_map = {r['name']: r for r in rooms_for_fc}
            fc_sel = st.selectbox('選擇教室', list(fc_rm_map.keys()), key='fc_rm_sel')
            fc_room = fc_rm_map[fc_sel]

            WEEKDAY_ZH_LIST = ['週一','週二','週三','週四','週五','週六','週日']

            # 新增固定課程
            with st.form('add_fc'):
                st.markdown("**新增固定課程**")
                c1, c2, c3 = st.columns(3)
                fc_wd  = c1.selectbox('星期', WEEKDAY_ZH_LIST, key='fc_wd')
                fc_st  = c2.time_input('開始時間', value=time(8, 0), key='fc_st')
                fc_et  = c3.time_input('結束時間', value=time(10, 0), key='fc_et')
                fc_ttl = st.text_input('課程名稱 *', key='fc_ttl', placeholder='例：土木概論（必填）')
                fc_nt  = st.text_input('備註（選填）', key='fc_nt')
                if st.form_submit_button('➕ 新增固定課程', use_container_width=True):
                    if not fc_ttl:
                        st.error('請填寫課程名稱')
                    elif fc_st >= fc_et:
                        st.error('結束時間必須晚於開始時間')
                    else:
                        wd_idx = WEEKDAY_ZH_LIST.index(fc_wd)
                        add_fixed_course(fc_room['room_id'], wd_idx,
                                         str(fc_st)[:5], str(fc_et)[:5],
                                         fc_ttl, fc_nt)
                        st.success(f'✅ 已新增：{fc_sel} {fc_wd} {str(fc_st)[:5]}～{str(fc_et)[:5]} {fc_ttl}')
                        st.rerun()

            # 現有固定課程列表
            existing_fc = get_fixed_courses(fc_room['room_id'])
            if existing_fc:
                st.markdown(f"**{fc_sel} 現有固定課程（{len(existing_fc)} 筆）**")
                for fc in existing_fc:
                    cc1, cc2, cc3 = st.columns([2, 3, 1])
                    cc1.write(f"**{WEEKDAY_ZH_LIST[fc['weekday']]}**　{fc['start_time']}～{fc['end_time']}")
                    cc2.write(f"📘 {fc['title']}" + (f"　*{fc['note']}*" if fc.get('note') else ''))
                    if cc3.button('🗑️ 刪除', key=f"dfc{fc['course_id']}"):
                        delete_fixed_course(fc['course_id']); st.rerun()
            else:
                st.info(f'{fc_sel} 尚未設定固定課程')
...
""")
    ...
    with c1:
        ...
        st.markdown("#### 📅 FIXED_COURSE（固定課程）")
        st.table(pd.DataFrame([
            ['course_id',  'INTEGER', 'PK 主鍵'],
            ['room_id',    'INTEGER', 'FK → ROOM'],
            ['weekday',    'INTEGER', '星期幾（0=週一…6=週日）'],
