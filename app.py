...
def book_room(user_id, room_id, book_date, st_, et_, purpose, note):
    ...
    db = conn()
    db.execute(
        "INSERT INTO ROOM_BOOKING VALUES (NULL,?,?,?,?,?,?,'confirmed',NULL,?,?)",
...
def get_equip_available(equip_id, book_date, st_, et_, exclude=None):
    ...
    q = ("SELECT COALESCE(SUM(quantity_borrowed),0) AS used FROM EQUIP_BOOKING "
         "WHERE equip_id=? AND book_date=? AND status='borrowed' "
         "AND start_time < ? AND end_time > ?" +
         (" AND booking_id != ?" if exclude else ""))
...
def all_equip_bookings():
    ...
    rows = db.execute("""SELECT eb.*, e.name AS equip_name, e.serial_number,
            e.quantity AS total_qty, u.username, u.department, u.student_id
        FROM EQUIP_BOOKING eb
        ...
        for b in bks:
            ...
            with st.expander(label):
                ...
                if b['status'] == 'confirmed':
                    c1, c2, c3 = st.columns(3)
                    if c1.button('🔓 歸還教室', key=f"ret{b['booking_id']}"):
                        return_room(b['booking_id'], user['user_id'])
                        st.success('已歸還，感謝！')
                        st.rerun()
...
""")
    ...
    with c3:
        ...
        st.table(pd.DataFrame([
            ...
            ['end_time',    'TEXT',    '結束時間'],
            ['purpose',     'TEXT',    '使用用途'],
            ['status',      'TEXT',    'confirmed / returned / cancelled'],
